...
const Comment = ({ roomId, comments }: { roomId: any | undefined, comments: any }) => {
    ...
    const addReview = ({
        ...
        if (comment) {
            axios.post('/reviews/create', { content: comment, rating: score, roomId, type: 0 })
                .then((res) => {
    ...
    return (
        <NavLabel className='place-details-reviews' name='reviews'>
            ...
            <Divider />
            <ReviewForm isComment={true} addReview={addReview} isStar={false} />
        </NavLabel>
    )
}
